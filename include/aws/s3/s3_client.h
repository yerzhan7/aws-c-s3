 ...
 *    allocation (zero-copy), set `request_body` (DEFAULT meta request only).
 */
struct aws_s3_meta_request_options {
     ...
     *
     * `request_body.ptr` / `request_body.len` are the bytes to send. `len` is taken as the request
     * body size and MUST match the request's Content-Length header.
     *
     ...
     * sends the body as a single (non-multipart) request.
     *
     * Because the body is always sent as a single request, it must stay within S3's single-request
     * size limit (5 GiB for PutObject); larger bodies will be rejected by S3.
     *
