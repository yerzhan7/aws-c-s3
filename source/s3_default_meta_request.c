...
static struct aws_future_void *s_s3_default_prepare_request(struct aws_s3_request *request) {
    ...
    if (meta_request_default->request_body.ptr != NULL && request->num_times_prepared == 0) {
         ...
         * request is fully torn down, which is signalled by the base shutdown callback
         * (aws_s3_meta_request_shutdown_fn), fired after this vtable destroy runs.
         *
         * Length was already validated against Content-Length at make_meta_request() time. */
